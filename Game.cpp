...
void Game::moveRight()
{
	if (_x < board->getXAt(board->Size() - 1, board->Size() - 1))
	{
...
void Game::moveDown()
{
	if (_y < board->getYAt(board->Size() - 1, board->Size() - 1))
	{
		Controller::playSound(MOVE_SOUND);
		if (board->getCheck(_x, _y) != _LOCK) {
			board->unselectedBlock(_x, _y);
...
int Game::checkIMatching(pair<int, int> firstBlock, pair<int, int> secondBlock, bool isChecking)
{
	...
	if (firstBlock.second == secondBlock.second) {
		...
		for (int i = firstBlock.first; i <= secondBlock.first; i += 4) {
	...
	if (firstBlock.first == secondBlock.first) {
		...
		for (int i = firstBlock.second; i <= secondBlock.second; i += 2) {
...
bool Game::checkZMatching(pair<int, int> firstBlock, pair<int, int> secondBlock, bool isChecking)
{
	...
	for (int i = firstBlock.first + 2; i < secondBlock.first; i += 2) {
		if (board->getCheck(i, firstBlock.second) == _DELETE) {
	...
	if (firstBlock.second > secondBlock.second)
		swap(firstBlock, secondBlock);
	for (int i = firstBlock.second + 1; i < secondBlock.second; i += 1) {
		if (board->getCheck(firstBlock.first, i) == _DELETE) {
...
bool Game::checkUMatching(pair<int, int> firstBlock, pair<int, int> secondBlock, bool isChecking)
{
	...
	if (firstBlock.first > secondBlock.first)
		swap(firstBlock, secondBlock);
	for (int i = firstBlock.first - 2; i >= x - 2; i -= 2) {
		Ucorner1.first = i;
	...
	if (firstBlock.first < secondBlock.first)
		swap(firstBlock, secondBlock);
	for (int i = firstBlock.first + 2; i <= x + size * 2; i += 2) {
		Ucorner1.first = i;
		Ucorner1.second = firstBlock.second;
		Ucorner2.first = i;
		Ucorner2.second = secondBlock.second;
		...
		if (i == x + size * 2) {
			Ucorner1.first = x + size * 2 - 2;
			Ucorner1.second = firstBlock.second;
			Ucorner2.first = x + size * 2 - 2;
			Ucorner2.second = secondBlock.second;
	...
	if (firstBlock.second > secondBlock.second)
		swap(firstBlock, secondBlock);
	for (int i = firstBlock.second - 1; i >= y - 1; i--) {
	...
	if (firstBlock.second < secondBlock.second)
		swap(firstBlock, secondBlock);
	for (int i = firstBlock.second + 1; i <= y + size; i++) {
		Ucorner1.first = firstBlock.first;
		Ucorner1.second = i;
		Ucorner2.first = secondBlock.first;
		Ucorner2.second = i;
		...
		if (i == y + size * 2) {
			Ucorner2.first = secondBlock.first;
			Ucorner2.second = y + size -1;
			Ucorner1.first = firstBlock.first;
			Ucorner1.second = y + size -1;
...
void Game::tobeContinue()
{
	...
	string str[2] = { "Yes", "No" };
	int left[] = { 35,40,47,58,63,69 }, word[] = { 32,32,175,174 }, color[] = { WHITE, GREEN }, top = 19;
	bool choice = 1;
	auto print1 = [&]()
	{
		int i = 0;
